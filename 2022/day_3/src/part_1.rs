...
pub fn run(){
    ...
    let rugsacks: Vec<Rugsack> = reader.lines().map(|line| {
        let rugsack = line.unwrap();
        let char_count = rugsack.chars().count();
        assert!(char_count % 2 == 0);

        Rugsack{compartment_1: rugsack[0..char_count/2].to_string(), compartment_2: rugsack[char_count/2..char_count].to_string()}
    }).collect();

    let total: Option<u32> = rugsacks.into_iter().map(|rugsack| get_char_priority(find_common_char(rugsack))).reduce(|p1, p2| p1+p2);
